pipeline {
   ...
   stage("Download Artifact from Nexus") {
        ...
        stage("Push Docker Image to Nexus") {
            steps {
                withDockerServer([uri: "http://192.168.33.10:8082/repository/docker-images-repo/"]) {
                    sh "docker login -u admin -p nexus 192.168.33.10:8081"
                    sh "docker tag kaddem-image http://192.168.33.10:8082/repository/docker-images-repo/kaddem-image:latest"
                    sh "docker push kaddem-image http://192.168.33.10:8082/repository/docker-images-repo/kaddem-image:latest"
                }
            }
        }
